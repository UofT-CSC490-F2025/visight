...
class InferencePipeline:
    ...
    def _run_model_inference(self, frames_metadata: list, frames_dir: Path, 
        ...
        try:
            ...
            batch_generator = self._split_into_batches(frames_metadata)
            num_batches = math.ceil(len(frames_metadata) / self.batch_size)
            results = []

            for batch_index, batch_dict in enumerate(batch_generator):
                # FIXED: Use batch_dict instead of batches[i]
                results.extend(
                    self._minibatch_inference(
                        batch_index, 
                        batch_dict["paths"], 
                        batch_dict["frame_meta"], 
                        annotated_dir
                    )
                )
    ...
    def _split_into_batches(self, frames_metadata: list): 
        """
        OPTIMIZED: Generator-based batching for memory efficiency
        """
        num_batches = math.ceil(len(frames_metadata) / self.batch_size)
        ...
        for i in range(num_batches): 
            ...
            # OPTIMIZATION: Yield instead of append (generator pattern)
            yield {
                "paths": [f["file_path"] for f in metadata_slice],
