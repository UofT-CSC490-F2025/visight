...
class InferencePipeline:
    ...
    def run_inference_on_video(self, video_path: str, video_id: str,
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            annotated_dir = Path(tmp_dir) / \
                "annotated" if save_annotated_frames else None
            if annotated_dir:
                annotated_dir.mkdir(parents=True, exist_ok=True)

            frames_metadata = self.video_processor.extract_frames(
            ...
            if upload_to_s3 and save_annotated_frames and annotated_dir:
                ...
                print(
                    f"Uploaded {len(inference_results)} annotated frames to S3")
            ...
            if upload_to_s3:
                ...
                results_bytes = json.dumps(
                    pipeline_results, indent=2).encode('utf-8')
        ...
        print(
            f"Frames with detections: {summary_stats['frames_with_detections']}, out of {summary_stats['total_frames']} frames")
        print(f"{'='*60}\n")
    ...
    def _run_model_inference(self, frames_metadata: list, frames_dir: Path,
        ...
        try:
            ...
            effective_bs = self.batch_size if (
                self.batch_size and self.batch_size > 1) else None
            ...
            if not effective_bs:
                for frame_meta in tqdm(frames_metadata, desc="Inference"):
                    ...
                    detection_info = [{"bbox": d[:4], "confidence": d[4],
                                       "class_name": name_map[d[5]]} for d in detection_data]
                    result = {
                    ...
                    if annotated_dir and len(detection_info) > 0:
                        annotated_path = self._draw_bounding_boxes(
                            frame_path, detection_info, annotated_dir)
            ...
            for batch in tqdm(list(chunk(frames_metadata, effective_bs)), desc=f"Inference (batch={effective_bs})"):
                ...
                all_dets = self.model.predict(
                    paths, conf=self.confidence_threshold, verbose=False, stream=True)
                i = 0
                for det in all_dets:
                    name_map = det.names
                    ...
                    if annotated_dir and len(detection_info) > 0:
                        annotated_path = self._draw_bounding_boxes(
                            Path(meta["file_path"]), detection_info, annotated_dir)
                        result["annotated_frame_path"] = str(annotated_path)
                    results.append(result)
                    i += 1
            return results
            ...
            profile_file = profile_output_dir / \
                f"run_model_inference_{timestamp}.txt"
    ...
    def _draw_bounding_boxes(self, frame_path: Path, detections: list,
        ...
        # OPTIMIZATION 1: Define constants once
        ...
        for det in detections:
            # OPTIMIZATION 2: Convert bbox coords to int once
            x1, y1, x2, y2 = [int(coord) for coord in det["bbox"]]
            class_name = det["class_name"]
            confidence = det["confidence"]

            # Draw bounding box (green)
            cv2.rectangle(img, (x1, y1), (x2, y2), COLOR, FONT_THICKNESS)
    ...
    def _generate_summary_stats(self, inference_results: list) -> dict:
        ...
        profiler.enable()
        try:
            # OPTIMIZATION 1: Use defaultdict to eliminate .get() calls
            ...
            # OPTIMIZATION 2: Single pass through data
            for result in inference_results:
                ...
                # OPTIMIZATION 3: Cleaner aggregation without redundant lookups
                for det in detections:
                    cls = det["class_name"]
                    class_counts[cls] += 1
                    class_conf_sum[cls] += float(det["confidence"])
            
            # OPTIMIZATION 4: Compute averages efficiently
            avg_confidence_per_class = {
                cls: class_conf_sum[cls] / count 
            ...
            return {
                ...
                "detection_rate": frames_with_detections / total_frames if total_frames else 0,
                "avg_detections_per_frame": total_detections / total_frames if total_frames else 0,
                "class_counts": dict(class_counts),  # Convert back to regular dict
            ...
            # Save profile to a profiling directory
            timestamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
            profile_output_dir = Path("profiling")
            profile_output_dir.mkdir(parents=True, exist_ok=True)
            profile_file = profile_output_dir / \
                f"summary_stats_{timestamp}.txt"
            profile_file.write_text(s.getvalue())
            print(
                f"\n[PROFILING] _generate_summary_stats profile saved to: {profile_file}")


def main():
    parser = argparse.ArgumentParser(
        description="Run inference pipeline on video")
    parser.add_argument("--video", required=True, help="Path to video file")
    parser.add_argument("--video_id", help="Video ID (defaults to filename)")
    parser.add_argument("--model_key", type=str,
                        default="models/raw-yolov8s-20251009-171047/best.pt", help="S3 key to model.pt")
    parser.add_argument(
        "--bucket", default="visight-data-yusufmoola", help="S3 bucket name")
    parser.add_argument(
        "--fps", type=int, help="Target FPS for frame extraction (default: all frames)")
    parser.add_argument("--confidence", type=float, default=0.5,
                        help="Confidence threshold for detections")
    parser.add_argument("--batch_size", type=int, default=0,
                        help="Batch size for batched inference (0 or 1 = per-frame)")
    ...
    pipeline = InferencePipeline(
        ...
        batch_size=(
            args.batch_size if args.batch_size and args.batch_size > 1 else None)
    )
